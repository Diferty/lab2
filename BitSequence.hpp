...
#include <iostream>   // для operator<< (опционально)
...
public:
    ...
    Bit GetFirst() const { return seq.GetFirst(); }
    Bit GetLast()  const { return seq.GetLast();  }
    Bit Get(int index) const { return seq.Get(index); }
    ...
    void Append(Bit b)   { seq.Append(b); }
    void Prepend(Bit b)  { seq.Prepend(b); }
    void InsertAt(Bit b, int index) { seq.InsertAt(b, index); }
    void Concat(const BitSequence& other) { seq.Concat(other.seq); }
    ...
    BitSequence operator&(const BitSequence& other) const { return And(other); }
    BitSequence operator|(const BitSequence& other) const { return Or(other); }
    BitSequence operator^(const BitSequence& other) const { return Xor(other); }
    BitSequence operator~() const { return Not(); }
};